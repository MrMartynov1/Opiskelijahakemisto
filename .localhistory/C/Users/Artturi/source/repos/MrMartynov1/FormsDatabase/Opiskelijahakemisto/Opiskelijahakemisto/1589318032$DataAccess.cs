...
namespace Opiskelijahakemisto
{
    public class DataAccess
    {
        ...
        public List<Opiskelija> GetNextOp(int currentOpid)
        {
            ...
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(Connecc.ConnValue("KoulutusTK")))
            {
                ...
                if (output.Count() > 0)
                {
                    ...
                    List<Opiskelija> op = connection.Query<Opiskelija>("dbo.Opiskelija_By_OpID @OpID", new { OpID = asd }).ToList();

                } else {
                    List<Opiskelija> op = connection.Query<Opiskelija>("dbo.Opiskelija_By_OpID @OpID", new { OpID = currentOpid }).ToList();
