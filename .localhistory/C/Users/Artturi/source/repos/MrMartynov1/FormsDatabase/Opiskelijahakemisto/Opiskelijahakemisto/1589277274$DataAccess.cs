...
namespace Opiskelijahakemisto
{
    public class DataAccess
    {
        ...
        public void PaivitaOpiskelija(int opid, string etunimi, string sukunimi, string hlotunnus, string yritys, string thnimike, string sposti, string osoite, string kaupunki, int postnum, string maa, string puh, string puh2, string site, string muuta)
        {
            ...
            {
                List<Opiskelija> opiskelijat = new List<Opiskelija>();
                /*var affectedRows =*/connection.Execute("dbo.Opiskelija_Update @OpID, @Etunimi, @Sukunimi, @Hlotunnus, @Yritys, @Thnimike, @Sposti, @Osoite, @Kaupunki, @Postnum, @Maa, @Puh, @Puh2, @Site, @Muuta",
                /*new { OpID = opid, Etunimi = etunimi, Sukunimi = sukunimi, Hlotunnus = hlotunnus, Yritys = yritys, Thnimike = thnimike, Sposti = sposti, Osoite = osoite, Kaupunki = kaupunki, Postnum = postnum, Maa = maa, Puh = puh, Puh2 = puh2, Site = site, Muuta = muuta },*/ opiskelijat);
        ...
        public void PaivitaKurssi(int krid, string krnimi)
        {
            ...
            {
                List<Kurssi> kurssit = new List<Kurssi>();
                connection.Execute("dbo.Kurssi_Update @OpID, @Etunimi, @Sukunimi, @Hlotunnus, @Yritys, @Thnimike, @Sposti, @Osoite, @Kaupunki, @Postnum, @Maa, @Puh, @Puh2, @Site, @Muuta",
                /*new { KurssiID = krid, Nimi = krnimi },*/ kurssit);
        ...
        public void PaivitaKoe(int koeid, string koenimi, string kieli, int vaikeus, int krid, string ope, int pto, string pvm)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(Connecc.ConnValue("KoulutusTK")))
            {
                ...
                //new { KoeID = koeid, Koenimi = koenimi, Kieli = kieli, Vaikeus = vaikeus, KurssiID = krid, Opettaja = ope, Prosenttitavoite = pto, Pvm = pvm }, kokeet);
        ...
        public void PaivitaTulos(int vtid, int opid, int pso, int koeid)
        {
            ...
            {
                List<Vastaukset> tulokset = new List<Vastaukset>();
                connection.Execute("dbo.Koe_Update @Id, @Opiskelija, @Prosenttimaara, @Koe",
                /*new { Id = vtid, Opiskelija = opid, ProsenttiMaara = pso, Koe = koeid },*/ tulokset);
                //Are Ids necessary? PROB NOT
            }
        }

        //
        public void PoistaOpiskelija(int opid, string etunimi, string sukunimi, string hlotunnus, string yritys, string thnimike, string sposti, string osoite, string kaupunki, int postnum, string maa, string puh, string puh2, string site, string muuta)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(Connecc.ConnValue("KoulutusTK")))
            {
                List<Opiskelija> opiskelijat = new List<Opiskelija>();
                /*var affectedRows =*/
                connection.Execute("dbo.Opiskelija_Delete @OpID", opiskelijat);
                /*new { OpID = opid, Etunimi = etunimi, Sukunimi = sukunimi, Hlotunnus = hlotunnus, Yritys = yritys, Thnimike = thnimike, Sposti = sposti, Osoite = osoite, Kaupunki = kaupunki, Postnum = postnum, Maa = maa, Puh = puh, Puh2 = puh2, Site = site, Muuta = muuta },*/
