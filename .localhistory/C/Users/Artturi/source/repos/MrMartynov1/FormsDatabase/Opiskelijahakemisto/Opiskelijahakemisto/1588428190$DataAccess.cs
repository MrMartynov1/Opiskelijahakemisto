...
namespace Opiskelijahakemisto
{
    ...
    public class DataAccess
    {
        public List<Opiskelija> HaeOpiskelijat(string sukunimi)
        {
            ...
            {
                //var output = connection.Query<Opiskelija>($"select * from Opiskelijat where Sukunimi = '{sukunimi}'").ToList();
        ...
        public int AvaaOpiskelija_ID(int opid)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(Connecc.ConnValue("KoulutusTK")))
            {
                var ret1 = connection.Query<Opiskelija>("dbo.Opiskelija_By_OpID @OpID", new { OpID = opid }).ToString();
                int reti1 = int.Parse(ret1);
                return reti1;
            }
        }
        ...
        */
        public void AvaaOpiskelija(int opid, Form1 form)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(Connecc.ConnValue("KoulutusTK")))
            {

                //Form1.tbHloT.Text = "test";
                //string tbhlot = form.fsd;
                //form.fsd = "asd";
            }
        }
