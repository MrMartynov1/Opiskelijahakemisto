...
namespace Opiskelijahakemisto
{
    public class DataAccess
    {
        ...
        public List<Koe> HaeKoe(string koenimi)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(Connecc.ConnValue("KoulutusTK")))
            {
                var output = connection.Query<Koe>("dbo.Opiskelija_By_LastName @Koenimi", new { Koenimi = koenimi }).ToList();
                var output2 = connection.Query<Koe>("dbo.Opiskelija_By_LastName @Koenimi", new { Koenimi = koenimi }).ToList();
                return output;
            }
        }
