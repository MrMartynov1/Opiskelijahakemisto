...
namespace Opiskelijahakemisto
{
    public class DataAccess
    {
        ...
        public List<Opiskelija> HaeKurssi(string nimi)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(Connecc.ConnValue("KoulutusTK")))
            {
                var output = connection.Query<Opiskelija>("dbo.Kurssi_By_Nimi @Nimi", new { Nimi = nimi }).ToList();
                return output;
            }
        }

        public List<Opiskelija> HaeKoe(string koenimi)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(Connecc.ConnValue("KoulutusTK")))
            {
                var output = connection.Query<Opiskelija>("dbo.Opiskelija_By_LastName @Sukunimi", new { Sukunimi = koenimi }).ToList();
                return output;
            }
        }
