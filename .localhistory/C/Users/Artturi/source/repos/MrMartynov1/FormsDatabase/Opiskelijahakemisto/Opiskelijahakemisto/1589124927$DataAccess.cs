...
namespace Opiskelijahakemisto
{
    public class DataAccess
    {
        ...
        public List<Vastaukset> HaeTulos(int koeid)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(Connecc.ConnValue("KoulutusTK")))
            {
                var output = connection.Query<Vastaukset>("dbo.Vastaukset_By_Koe @Koe", new { Koe = koeid }).ToList();
                return output;
            }
        }

        public List<Vastaukset> HaeTulos2(int koeid, int opid)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(Connecc.ConnValue("KoulutusTK")))
            {
                ...
                {
                    //var output1 = connection.Query<Vastaukset>("dbo.Vastaukset_By_Koe_And_Opiskelija @Koe", new { Koe = koeid }).ToList();
                    //var output2 = connection.Query<Vastaukset>("dbo.Vastaukset_By_Koe_And_Opiskelija @Opiskelija", new { Opiskelija = opid }).ToList();
