...
namespace Opiskelijahakemisto
{
    ...
    {
        public List<Kurssi> HaeOpiskelijat(string sukunimi)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(Connecc.ConnValue("KoulutusTK")))
            {
                var output = connection.Query<Kurssi>("dbo.Opiskelija_By_LastName @Sukunimi", new { Sukunimi = sukunimi }).ToList();
                return output;
            }
        }
        ...
        public List<Kurssi> HaeKoe(string koenimi)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(Connecc.ConnValue("KoulutusTK")))
            {
                var output = connection.Query<Kurssi>("dbo.Opiskelija_By_LastName @Sukunimi", new { Sukunimi = koenimi }).ToList();
                return output;
            }
        }
